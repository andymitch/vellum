<script lang="ts">
  ...
  // render over the chunk's own content; on a pane too narrow to have a margin
  // it's a full-width day rule between chunks instead (#242).
  //
...
<style>
  ...
  /* Day label and exact times occupy the same spot and cross-fade, so the
     ...
     Bounded by the column's right edge rather than sized to the text: a
     timestamp is easily wider than the margin at its narrowest (7rem, of
     which 2.4rem is the offset to here), and text that doesn't wrap spills
     out of the pane and gives it a horizontal scrollbar. Wrapping instead
     costs nothing — everything here is absolutely positioned, so a second
     line can't push the row's own content around. */
  .rail-day,
  .rail-times {
    position: absolute;
    left: 2.4rem;
  ...
  /* Below 55rem the right margin is narrower than the ~7rem a rail needs to
     say anything useful, so it isn't shown at all: the chunk pins to the left
     edge and a bare right margin grows on its own from nothing, until the
     62rem rule above takes over and the rail appears already at full width.
     The day rule takes over as the timeline here (see .day-rule). */
  ...
  /* A day change, wherever there's no margin to put a rail in: a full-width
     rule with the label set off the end of it (#242). Same visual language as
     the pre-notebook Journal's inline dated rule. Hidden until the pane is too
     narrow for a rail (below), so both forms of the timeline are driven by the
     same measurement — the pane's width — and exactly one of them shows at any
     width. Keying this off a viewport media query instead would leave a narrow
  ...
  /* Must come after .day-rule's own `display: none` to win on source order —
     the same width at which the rail above gives up. */
  @container (max-width: 55rem) {
    .day-rule {
      display: flex;
    }
